...
public class FileManager {
    ...
    private final String[] titles = {"titulo","autor","anio","estante_numero","estante_seccion","piso","edificio","sede"
    };
    /*
    ...
    public HashSet<Libro> getData() {
        List beans = null;
        try {
            FileReader ff = new FileReader(theFile, StandardCharsets.UTF_8);
    ...
    public void saveData(){
        try {
            ...
            var inventario = DataManager.getInstance().getData();

            for(Libro l : inventario){
                //significa que lo quiero mantener
                writer.writeNext(l.getDataToCsv(),false);
            }
            writer.close();
